...
function onGotiClick(e) {
    ...
    // replicate exactly the original inlined behavior
    parentBox.click();
    touchtime = 1;
    ...
    if (turn[currentTurn] == goticolor) {
        // use the live selectElement reference
...
function onSelectMouseDown(e) {
    console.log(assignedColor[socket.id], turn[currentTurn]);
    if (assignedColor[socket.id] !== turn[currentTurn]) {
        e.preventDefault(); // stops default action
        return; // exits without running your logic
    }
}

// initial bind
selected = document.querySelector(".select");
selected.removeEventListener("mousedown", onSelectMouseDown);
selected.addEventListener("mousedown", onSelectMouseDown);
...
function rebindDOMAfterReplace() {
    ...
    for (const g of gotiyan) {
        g.removeEventListener('click', onGotiClick);
...
// rebind initially so the goti handlers are attached to the initial DOM
...
function alignContainer() {
    ...
    // Remove any previously assigned color class
    container.classList.remove("GreenPlayer", "RedPlayer", "BluePlayer");

    // Add the new color class based on assigned color
    if (getColor === "green") {
        container.classList.add("GreenPlayer");
    } else if (getColor === "red") {
        container.classList.add("RedPlayer");
    } else if (getColor === "blue") {
        container.classList.add("BluePlayer");
    }
}
